...
test('Main', () => {
  ...
  for (const packageJsonFile of [
    ...
    assert.equal(
      getPackageDependencies(packageJsonFile),
      dependencies
    )
  }

  assert.deepEqual(dependencies.data, packageJson)
  assert.equal(dependencies.peerDependencies.size, Object.keys(packageJson.peerDependencies ?? {}).length)
  assert.equal(dependencies.devDependencies.size, Object.keys(packageJson.devDependencies ?? {}).length)
})
...
const fixtures = new URL('./fixtures/', import.meta.url)
  ...
  await spawn('yarn',['--immutable'], {cwd: fixtures, env: {YARN_ENABLE_IMMUTABLE_INSTALLS:'true'}})
  const fixturesPackage = getPackageDependencies(fixtures)
  const {
    data: fixturesPackageJson,
    file: fixturesPackageJsonFile
  } = fixturesPackage
...

  {
    ...
    assert.equal(jsrDependency.file, url.fileURLToPath(new URL('./node_modules/@std/path/package.json', fixtures)))
    assert.equal(jsrDependency.resolved.name, '@jsr/std__path')
    assert.equal(jsrDependency.resolved.version, '1.1.4')

    const deepDependency = jsrDependency.resolved.dependencies.get('@jsr/std__internal')
    assert.ok(deepDependency)
    assert.equal(deepDependency.base, jsrDependency.file)
  }

  {
    const renamedDependency = fixturesPackage.devDependencies.get('renamed-deno-path-from-file-url')
...

  {
    ...
    assert.equal(localDependency.resolved.name, '@package-dependencies-tree/local-package')
    assert.equal(localDependency.resolved.version, '1.0.0')

    const deepDependency = localDependency.resolved.peerDependencies.get('@std/path')
    ...
    assert.equal(deepDependency.resolved, fixturesPackage.dependencies.get('@std/path').resolved)
  }
})
